...
export default function RegisterPage() {
  ...
  // Charger les classes si rôle = étudiant
  useEffect(() => {
    if (role === "student") {
  ...
  // Charger les matières si rôle = professeur
  useEffect(() => {
    if (role === "professor") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (role === "membersite") {
        ...
        const fd = new FormData();
        ["username","email","password1","password2","first_name","last_name",
         "department","hire_date"].forEach(k => { if (form.get(k)) fd.append(k, form.get(k)); });
      ...
      <div style={{ maxWidth: 900, margin: "24px auto", padding: "0 16px" }}>
        ...
        {role && (
          <form onSubmit={handleSubmit} style={{ display: "grid", gap: 12, marginTop: 12 }}>
            ...
            {role === "alumni" && (
              <>
                <input name="year_left" type="number" min="1995" max="2100" placeholder="Année de sortie" required />
